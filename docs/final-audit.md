...
| A3 | Frontend stack fixed to React + TypeScript + Vite + React Flow + Zustand | implemented | `npm run typecheck`, `npm run build --workspace @causal-graph-workbench/frontend` | `apps/frontend/package.json`, `apps/frontend/src/components/GraphCanvas.tsx`, `apps/frontend/src/store/useWorkbenchStore.ts`, `apps/frontend/vite.config.ts` | Matches the requested stack. |
...
| A66 | Product should feel like a complete local web app rather than a static mockup | implemented | `npm run build --workspace @causal-graph-workbench/frontend`, startup smoke-run | `apps/frontend/src/*`, `apps/backend/app/*`, `README.md` | Frontend and backend are connected through live API calls. |
